...
module ApolloFederation
  module Schema
    IMPORTED_DIRECTIVES = ['inaccessible', 'tag', 'requiresScopes'].freeze
    ...
    module CommonMethods
      ...
      def federation_2_prefix
        ...
        <<~SCHEMA
          extend schema
            @link(url: "https://specs.apollo.dev/federation/v2.5"#{federation_namespace}, import: [#{(IMPORTED_DIRECTIVES.map { |directive| "\"@#{directive}\"" }).join(', ')}])
